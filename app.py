...
def index():
    if request.method == "POST":
        ...
        s2 = "The Risk of Credit Card Default based on Regression is " + str(pred2[0])
        
        model3 = joblib.load("CCD_NN")
        pred3 = model3.predict([[income, age, loan]])
        s3 = "The Risk of Credit Card Default based on Neural Network is " + str(pred3[0])
        
        model4 = joblib.load("CCD_GB")
        pred4 = model4.predict([[income, age, loan]])
        s4 = "The Risk of Credit Card Default based on Gradient Boosting is " + str(pred4[0])
        
        model5 = joblib.load("CCD_RF")
        pred5 = model5.predict([[income, age, loan]])
        s5 = "The Risk of Credit Card Default based on Random Forest is " + str(pred5[0])
